package io.kito.kore.datagen

import io.kito.kore.reflect.ObjectScanner
import io.kito.kore.reflect.Scan
import io.kito.kore.registry.BlockRegister.BlockBuilder
...
abstract class DataGenHelper(private val modId: String) {
    ...
    fun <T : Block> BlockBuilder<T>.state(builder: BlockStateBuilder<T>)
        { blockModelBuilders += loc(modId, name) to (builder as BlockStateBuilder<Block>) }
    ...
    fun BlockBuilder<*>.named(vararg entries: Pair<String, String>)
        { entries.forEach { translationEntries.computeIfAbsent(it.first) { arrayListOf() } +=
            { add(BLOCK[loc(modId, this@named.name)], it.second) } } }
