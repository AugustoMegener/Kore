...
abstract class ScriptValueReloadListener<T>(script: KotlinType) : ScriptReloadListener(script) {
                    ...
                    when(res) {
                        ...
                        is ResultWithDiagnostics.Success<EvaluationResult> ->
                            when(val value = res.value.returnValue) {
                                ...
                                is ResultValue.Unit -> error("$loc Script returns Unit, value expected")
                                is ResultValue.Value -> (value.value as? T)?.let { put(loc, it) }
                            }
                    }
                }
            }
